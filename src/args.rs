...
#[structopt(author)]
pub struct Args {
    /// The separator between columns. Must be a single ASCII character, or an accepted special sequence. Default: ,
    ///
    /// The special sequences are the following
    ///
    ...
    pub separator: u8,
    /// Terminator of a record. Follows the same syntax as delimiters. The default value (an extra
    /// special sequence `crlf`) accepts any of `\r`, `\n`, or `\r\n`.
    #[structopt(short = "r", long, default_value = "crlf", parse(try_from_str = terminator))]
    pub terminator: Terminator,
    /// Lines beginning with this comment symbol are skipped. Comments are disabled by default.
    #[structopt(short, long, parse(try_from_str = delimiter))]
    pub comment: Option<u8>,
    /// Whether this CSV has a header in the first row.
    #[structopt(short="H", long)]
    pub headers: bool,
    /// Whether to trim leading/trailing whitespace from `headers`, `fields`, or both (`all`).
    /// No trimming is performed by default.
    #[structopt(short, long, parse(from_str = trim))]
    pub trim: Trim,
    /// The quote character to use when parsing. Follows the same syntax as delimiters. Default: "
    #[structopt(short, long, default_value = "\"", parse(try_from_str = delimiter))]
    pub quote: u8,
